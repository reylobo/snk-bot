...
# Default settings for old versions of settings.py:
...
async def on_message(message):
    ...
    if rateLimitResult == RequestLimitResult.ALREADY_OVER_LIMIT:
        ...
        await message.channel.send(message.channel, "Error: Total daily bot request limit reached. Try again in 24 hours.")
        return
    elif rateLimitResult == RequestLimitResult.USER_LIMIT_JUST_REACHED:
        await message.channel.send(message.channel, "Error: Daily request limit reached for user " + message.author.name + ". Try again in 24 hours.")
        return
